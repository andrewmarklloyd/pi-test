...
response=$(curl -s -X POST -H "api-key: ${PI_APP_DEPLOYER_API_KEY}" -d "{\"sha\":\"${GITHUB_SHA}\",\"repository\":\"andrewmarklloyd/pi-test\",\"name\":\"app_${GITHUB_SHA}\",\"manifest_name\":\"pi-test-arm\"}" https://pi-app-deployer.herokuapp.com/push)
echo ${response}
